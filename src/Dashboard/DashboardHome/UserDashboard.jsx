...
const UserDashboard = () => {
  ...
  const { data: totalRevenue = [], isLoading } = useQuery({
    ...
    // enabled: !!user?.email,
    queryFn: async () => {
      const { data } = await axiosSecure(`/users-revenue?email=${user?.email}`);
      // console.log(data);
      return data;
    },
  });
  const { data: userWeeklyRevenue = [] } = useQuery({
    queryKey: ["userWeeklyRevenue"],
    // enabled: !!user?.email,
    queryFn: async () => {
      const { data } = await axiosSecure(
        `/users-weekly-revenue?email=${user?.email}`
      );
      // console.log(data);
      return data;
    },
  ...
  const weeklyDate = userWeeklyRevenue.map((item) => ({
    date: item._id,
